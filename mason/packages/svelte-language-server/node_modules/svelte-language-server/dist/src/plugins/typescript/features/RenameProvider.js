...
class RenameProviderImpl {
    ...
    async rename(document, position, newName) {
        ...
        convertedRenameLocations = this.checkShortHandBindingOrSlotLetLocation(lang, convertedRenameLocations, docs, newName);
    ...
    async mapAndFilterRenameLocations(renameLocations, snapshots, newName) {
        const mappedLocations = await Promise.all(renameLocations.map(async (loc) => {
            ...
            if (!(snapshot instanceof DocumentSnapshot_1.SvelteDocumentSnapshot) ||
                (!(0, utils_3.isTextSpanInGeneratedCode)(text, loc.textSpan) &&
    ...
    checkShortHandBindingOrSlotLetLocation(lang, renameLocations, snapshots, newName) {
        return renameLocations.map((location) => {
            const sourceFile = lang.getProgram()?.getSourceFile(location.fileName);
            if (!sourceFile ||
                location.fileName !== sourceFile.fileName ||
    ...
    transformShorthand(snapshot, location, newName) {
    ...
    getBindingOrAttrShorthand(snapshot, position, svelteNode = snapshot.svelteNodeAt(position)) {
        if ((svelteNode?.parent?.type === 'Binding' ||
